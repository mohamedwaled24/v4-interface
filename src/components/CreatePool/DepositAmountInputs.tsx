...
const DepositAmountInputs: React.FC<DepositAmountInputsProps> = ({
  token0,
  token1,
  token0Amount,
  token1Amount,
  ...
  const handlePercentageClick = (tokenIndex: number, percentage: number) => {
    // In a real implementation, this would calculate the percentage of the user's balance
  ...
  return (
    <Container>
      ...
      <AmountInputContainer>
        ...
        />
        <TokenInfo>
          <TokenBalance>0 {token0?.symbol}</TokenBalance>
          <TokenDisplay>
            {token0?.logoURI ? (
              <img 
                src={token0.logoURI} 
      ...
      <AmountInputContainer>
        ...
        />
        <TokenInfo>
          <TokenBalance>0 {token1?.symbol}</TokenBalance>
          <TokenDisplay>
            {token1?.logoURI ? (
              <img 
                src={token1.logoURI} 
